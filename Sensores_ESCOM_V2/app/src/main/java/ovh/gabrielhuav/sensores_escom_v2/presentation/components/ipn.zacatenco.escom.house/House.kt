...
class House : AppCompatActivity(),
    ...
    // Add missing button declarations
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            mapView = MapView(
                context = this,
                mapResourceId = R.drawable.house_map // Make sure this drawable exists
            )
            ...
            mapView.post {
                mapView.setCurrentMap(MapMatrixProvider.MAP_HOUSE, R.drawable.house_map)
                mapView.playerManager.apply {
                    setCurrentMap(MapMatrixProvider.MAP_HOUSE)
    ...
    private fun initializeManagers() {
        ...
        mapView.setMapTransitionListener(object : MapView.MapTransitionListener {
    ...
    private fun handleBackButtonPressed() {
        if (gameState.isConnected) {
            serverConnectionManager.sendUpdateMessage(
                playerName,
                MapMatrixProvider.MAIN_TO_HOUSE_POSITION,
                MapMatrixProvider.MAP_MAIN
            )
        }
    ...
    private fun returnToMain() {
        val intent = Intent(this, GameplayActivity::class.java).apply {
            ...
            putExtra("INITIAL_POSITION", MapMatrixProvider.MAIN_TO_HOUSE_POSITION)
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            ...
            if (gameState.isConnected) {
                ...
                serverConnectionManager.sendUpdateMessage(playerName, position, MapMatrixProvider.MAP_HOUSE)

                // Log de debug para confirmar
                Log.d(TAG, "Sending update: Player $playerName at $position in map ${MapMatrixProvider.MAP_HOUSE}")
            }
        }
    ...
    override fun onPositionReceived(device: BluetoothDevice, x: Int, y: Int) {
        runOnUiThread {
            ...
            mapView.updateRemotePlayerPosition(deviceName, Pair(x, y), MapMatrixProvider.MAP_HOUSE)
            mapView.invalidate()
        }
    }
                    ...
                    "positions" -> {
                        ...
                        players.keys().forEach { playerId ->
                            if (playerId != playerName) {
                                ...
                                if (map == MapMatrixProvider.MAP_HOUSE) {
    ...
    private fun checkPositionForMapChange(position: Pair<Int, Int>) {
        when {
            position.first == 2 && position.second == 2 -> {
                // Punto de entrada a la casa
                val intent = Intent(this, InsideHouse::class.java).apply {
                    putExtra("PLAYER_NAME", playerName)
                    putExtra("IS_SERVER", gameState.isServer)

...
public class RoleManager {
    ...
    public boolean hasRole(String role) {
        ...
        // Verificar si estamos usando Firebase para la autenticación
        if (sessionManager.isFirebaseAuth()) {
            com.example.systembooks.firebase.FirebaseUser fbUser = sessionManager.getFirebaseUser();
            userRole = fbUser != null ? fbUser.getRole() : null;
        } else {
            // Autenticación API tradicional
    ...
    public String getCurrentRole() {
        ...
        if (sessionManager.isFirebaseAuth()) {
            com.example.systembooks.firebase.FirebaseUser fbUser = sessionManager.getFirebaseUser();
            role = fbUser != null ? fbUser.getRole() : null;
        } else {
            // Autenticación API tradicional
